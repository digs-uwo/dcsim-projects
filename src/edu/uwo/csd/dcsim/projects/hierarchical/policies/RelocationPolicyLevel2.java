 ...
 *
 */
public class RelocationPolicyLevel2 extends Policy {
	 ...
	 */
	public RelocationPolicyLevel2(AutonomicManager target) {
	...
	protected void searchForAppMigrationTarget(MigRequestEntry entry) {
		...
		// Could not find suitable target Rack in the Cluster.
			...
			// to find a target Rack for the migration in another Cluster.
	...
	protected void searchForVmMigrationTarget(MigRequestEntry entry) {
		...
		// then check if the Rack can host the VM; otherwise, activate a new Rack.
			...
			for (RackData rack : active) {
				...
				// If several Racks require a minimum number of Host activations,
				// pick the most loaded Rack among them.
				int hostActivations = RackData.calculateMinHostActivations(entry.getVm(), rack.getCurrentStatus().getStatusVector(), rack.getRackDescription().getHostDescription());
				if (hostActivations >= 0) {
					if (hostActivations < minHostActivations) {
						minHostActivations = hostActivations;
						mostLoaded = rack;
		...
		if (null != targetRack) {
			...
			simulation.getLogger().debug(String.format("[Cluster #%d] AppRelocationPolicyLevel2 - VM #%d - Found migration target: Rack #%d.",
					manager.getCapability(ClusterManager.class).getCluster().getId(),
					entry.getVm().getId(),
					targetRack.getId()));
		...
		// Could not find suitable target Rack in the Cluster.
			...
			if (null != rackPool.getRack(entry.getSender())) {
				...
				simulation.getLogger().debug(String.format("[Cluster #%d] AppRelocationPolicyLevel2 - VM #%d - Failed to find migration target. Contact DC Manager.",
						manager.getCapability(ClusterManager.class).getCluster().getId(),
						entry.getVm().getId()));
			...
			// Event's sender does not belong in this Cluster.
				...
				simulation.getLogger().debug(String.format("[Cluster #%d] AppRelocationPolicyLevel2 - VM #%d - Failed to find migration target. Reject migration request.",
						manager.getCapability(ClusterManager.class).getCluster().getId(),
						entry.getVm().getId()));
