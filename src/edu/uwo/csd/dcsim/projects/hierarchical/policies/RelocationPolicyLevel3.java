 ...
 *
 */
public class RelocationPolicyLevel3 extends Policy {

	/**
	 * Creates an instance of AppRelocationPolicyLevel3.
	 */
	public RelocationPolicyLevel3() {
	...
	public void execute(AppMigRequestEvent event) {
		...
		simulation.getLogger().debug(String.format("[DC Manager] RelocationPolicyLevel3 - New MigRequest - App #%d.", event.getApplication().getId()));
	...
	public void execute(AppMigRejectEvent event) {
		...
		simulation.getLogger().debug(String.format("[DC Manager] RelocationPolicyLevel3 - New Migration reject - App #%d.", event.getApplication().getId()));
	...
	protected void searchForAppMigrationTarget(MigRequestEntry entry) {
		...
		simulation.getLogger().debug("[DC Manager] RelocationPolicyLevel3 - Searching for target Cluster...");
		...
		if (active.size() == 1) {
			...
			for (ClusterData cluster : active) {
				...
				if (currentPowerEff != cluster.getClusterDescription().getPowerEfficiency()) {
					...
					if (null != leastLoadedRack
							&& leastLoadedRack.getCurrentStatus().getStatusVector() != null
							&& ClusterData.canHost(entry.getVm(), leastLoadedRack.getCurrentStatus().getStatusVector(), leastLoadedRack.getClusterDescription())) {
